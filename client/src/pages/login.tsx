...
const Login = () => {
  ...
  const handleSignIn = async (method: () => Promise<any>) => {
    try {
      setIsLoading(true);
      await method();
      // On success, better-auth usually redirects away, 
      // so we don't necessarily need to reset loading here 
      // unless it's a non-redirecting flow.
    } catch (error: any) {
      console.error('Sign in error:', error);
      toast({
        title: "로그인 실패",
        description: error.message || "로그인 시도 중 오류가 발생했습니다. 다시 시도해 주세요.",
        variant: "destructive"
      });
      setIsLoading(false);
    }
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="min-h-screen pt-40 pb-20 flex items-center justify-center bg-background"
      >
      <div className="container mx-auto px-4 max-w-md">
        <motion.div variants={fadeIn} className="text-center mb-8">
          <h1 className="text-4xl font-bold font-montserrat mb-2 text-foreground">
            <span className="bg-gradient-to-r from-primary to-accent text-transparent bg-clip-text">로그인</span>
          </h1>
